...
contract MiladyDrop is IMiladyDrop {
    ...
    function _setRequiredNFTAddresses(address[] memory  _requiredNFTAddresses) private {
    ...
    function _setClaimed(uint256 nftIndex, uint256 id) private {
        ...
        claimedBitMapPerNftIndex[nftIndex][claimedWordIndex] = claimedBitMapPerNftIndex[nftIndex][claimedWordIndex] | (1 << claimedBitIndex);
    }

    function isClaimed(uint256 nftIndex, uint256 id) public view override returns (bool) {
        uint256 claimedWordIndex = id / 256;
        uint256 claimedBitIndex = id % 256;
    ...
    function verifyClaim(uint256 nftIndex, uint256 id, uint256 amount, bytes32[] calldata merkleProof) private view {
    ...
    function claim(uint256 nftIndex,uint256 id, uint256 amount, bytes32[] calldata merkleProof)
        public
        virtual
        override
    {
    ...
    function _buildLeavesAndTotalAmount(uint256[] calldata ids, uint256[] calldata amounts, uint256[] calldata nftIndexes ) private returns (bytes32[] memory, uint256) {
    ...
    function claimMultiple(
        ...
        (bytes32[] memory leaves, uint256 totalAmount) = _buildLeavesAndTotalAmount(ids, amounts, nftIndexes);
